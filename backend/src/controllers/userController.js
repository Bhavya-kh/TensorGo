...
const createUser = async (req, res) => {
  try {
    ...
    if (!currentUser.subscriptionAdmin) {
      return res
        .status(403)
        .json({ message: "Only subscription admins can create users." });
    }
    ...
    if (!plan) {
      return res
        .status(404)
        .json({ message: "Plan not found for the reference user." });
    }
    if (referenceUser.otherUsers.length >= plan.userLimit) {
      return res
        .status(400)
        .json({ message: "User limit exceeded for the reference user." });
    }
  ...
  try {
    ...
    const formattedOrganizations = await Promise.all(
      organizations.map(async (org) => {
        const status =
          org.valid && org.valid > currentDate ? "Active" : "Inactive";
        ...
        return {
          ...
          lastBilling: org.valid
            ? org.valid.toISOString().split("T")[0]
            : "N/A",
          status,
          billingStatus,
        };
